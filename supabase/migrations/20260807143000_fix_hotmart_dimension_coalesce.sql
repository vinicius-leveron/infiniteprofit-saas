-- COALESCE is SQL syntax and cannot be schema-qualified. Recreate the
-- production function installed by 20260807130000 with valid expressions.
...
begin
  ...
  with hotmart_events as (
    ...
    where event.project_id = _project_id
      ...
      and pg_catalog.lower(coalesce(event.payload->>'provider', '')) = 'hotmart'
