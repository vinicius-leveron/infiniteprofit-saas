...
import { readHotmartImportFile } from "@/lib/hotmartImportFile";
import type { PreparedHotmartImport } from "@/lib/hotmartImport";
import { cn } from "@/lib/utils";

const ACCEPTED_HOTMART_FILES =
  ".csv,.txt,.xlsx,.xls,text/csv,text/plain,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel";

type HotmartImportPickerProps = {
  value: PreparedHotmartImport | null;
  onChange: (value: PreparedHotmartImport | null) => void;
  disabled?: boolean;
  compact?: boolean;
};

export function HotmartImportPicker({
  value,
  onChange,
  disabled = false,
  compact = false,
}: HotmartImportPickerProps) {
  const inputId = useId();
  ...
  async function handleFile(file: File | undefined) {
    ...
    setError(null);
    try {
      const result = await readHotmartImportFile(file);
      ...
      onChange({
        ...
        discardedColumns: result.discardedColumns,
      });
    } catch (caught) {
      onChange(null);
      setError(
        caught instanceof Error
          ? caught.message
          : "Não foi possível ler a planilha Hotmart.",
      );
    } finally {
      setReading(false);
    }
      ...
      >
        <div className="flex items-start gap-3">
          ...
          >
            ...
            <span className="sr-only">Remover arquivo da Hotmart</span>
          </Button>
        </div>
      </div>
    );
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-sm font-medium">
            {reading ? "Lendo o histórico…" : "Selecionar XLS, XLSX ou CSV da Hotmart"}
          </span>
          <span className="mt-0.5 block text-xs leading-4 text-muted-foreground">
            Relatório de vendas exportado pela Hotmart, com até 8 MB.
          </span>
        </span>
